...
NEW = []   # engineered feature names added to the model feature set, in order
...
#     clip(0,1): values > 1 are data-quality artefacts
...
#
# REMOVED: velocity_per_age  (r=0.97 with txn_velocity_log — near-duplicate)
# REMOVED: speed_exit_score  (r=-0.98 with time_to_first_outbound_minutes_log)
#
# burst_per_age is retained: burst_score has low correlation with
# account_age_days_log (r≈0.0), so burst_per_age carries genuinely new
# information about burst intensity relative to account maturity.
# ══════════════════════════════════════════════════════════════════════════════
...
#
# behavioral_risk_score, network_risk_score, overall_suspicion_score are
# weighted sums of features that also appear individually in the feature set.
# Including them gives XGBoost a pre-summarised label proxy, causing training
# to converge in < 10 rounds and inflating AUC artificially.
# They are not computed at all to prevent accidental inclusion downstream.
# ══════════════════════════════════════════════════════════════════════════════
...
#
# REMOVED: account_age_tier  (r=0.96 with account_age_days_log — near-duplicate)
# REMOVED: velocity_tier     (r=0.96 with txn_velocity_log — near-duplicate)
#
# Retained: kyc_risk_tier (thresholds have regulatory meaning distinct from
#           the continuous score), burst_tier, fund_exit_tier, alert_tier.
#           alert_tier has r=0.91 with alert_count — borderline, but the
#           tier encodes non-linear regulatory thresholds (0, 2, 5, 10 alerts)
#           that a linear correlation doesn't capture, so kept.
# ══════════════════════════════════════════════════════════════════════════════
...
#     Encodes regulatory alert thresholds, kept despite r=0.91 with alert_count
df['alert_tier'] = safe_cut(
    df['alert_count'],
    [-1, 0, 2, 5, 10, df['alert_count'].max() + 1],
    [0, 1, 2, 3, 4]
)
NEW.append('alert_tier')

# E2  kyc_risk_tier  [PMLA S.12A, RBI-KYC S.38]
#     0=low (<0.40), 1=medium (0.40-0.70), 2=high (>0.70, EDD mandatory)
...
# ── SAVE ──────────────────────────────────────────────────────────────────────
out = r'C:\Users\shriy\hack-o-hire\data_engineered.csv'
...
print(f"  Columns : {len(final_cols)}"
      f"  ({len(ORIG_FEAT)} orig + {len(CBRT_FEAT)} cbrt"
      f" + {len(LOG1P_FEAT)} log + {len(NEW)} engineered + {len(LABEL_COLS)} labels)")
